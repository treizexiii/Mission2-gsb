...
class PdoGsb
{
	...
	{
		$req = "select visiteur.id as id, visiteur.nom as nom, visiteur.prenom as prenom, role.roleName as statut from visiteur INNER JOIN role ON visiteur.roleId = role.id
	...
	{
		$req = "select * FROM visiteur INNER JOIN role WHERE visiteur.roleId = role.id AND role.id = 1";
		$res = $this->monPdo->query($req);
		$lesLignes = $res->fetchAll();
		return $lesLignes;
	}
